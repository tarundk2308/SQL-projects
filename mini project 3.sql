...
insert into account values (1,1,1,10000,'savings','2012-12-15','active');
insert into account values (2,2,2,5000,'savings','2012-06-12','active');
...
update employee set fname = 'karan',lname = 'singh' where fname = 'bella';
...
select c.custid,c.fname,a.curbal
...
select count(custid),occupation from customer group by occupation;
...
select lname from employee where lname like '%a';

# 14.	Select the details of the employee who work either for department ‘loan’ or ‘credit’
select fname from employee where dept in ('loan','credit');
...
bid int,
amount int,
type_trans varchar(30) check (type_trans in ('withdrawal','deposit')),
dot date,
foreign key (custid) references customer (custid),
...
create trigger bal_update after update on transaction_table  for each row
case
when type_trans = 'deposit' then update account set old.curbal = old.curbal + amount where old.acnumber = account_no
when type_trans = 'withdrawal' then update account set old.curbal = old.curbal - amount where old.acnumber = account_no
end;


# 19.	Write a query to display the details of customer with second highest balance 
select max(curbal),(select fname from customer where custid = a.custid) from account a where curbal < (select max(curbal) from account) group by custid;
...
select product_desc,product_class_code,product_price,
case
...
select p.product_desc,(select product_class_desc from product_class where product_class_code = p.product_class_code) as class_description,
p.product_price from product p where p.product_id in (select product_id from order_items 
where order_id in (select order_id from order_header where order_status ='shipped' ));
...
case
when pc.product_class_desc in ('electronics','computer') and p.product_quantity_avail < 10 then 'low_stock'
when pc.product_class_desc in ('electronics','computer') and 11 < p.product_quantity_avail < 30 then 'in_stock'
when pc.product_class_desc in ('electronics','computer') and p.product_quantity_avail > 31 then 'enough_stock'
when pc.product_class_desc in ('stationery','clothes') and p.product_quantity_avail < 20 then 'low_stock'
when pc.product_class_desc in ('stationery','clothes') and 21 < p.product_quantity_avail < 80 then 'in_stock'
when pc.product_class_desc in ('stationery','clothes') and p.product_quantity_avail > 81 then 'enough_stock'
when pc.product_class_desc not in ('electronics','computer','stationery','clothes') and p.product_quantity_avail < 15 then 'low_stock'
when pc.product_class_desc not in ('electronics','computer','stationery','clothes') and 16 < p.product_quantity_avail < 50 then 'low_stock'
when pc.product_class_desc not in ('electronics','computer','stationery','clothes') and p.product_quantity_avail > 51  then 'enough_stock'
...
select distinct(oc.customer_fname)
